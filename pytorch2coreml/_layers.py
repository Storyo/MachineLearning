...
def _convert_residual(builder, name, layer, input_names, output_names):
    layers = layer.named_children
    n = len(layer._modules)

    print("Layer Modules: ", n)
    ...
    for name, l in layer.named_children():
        ...
        print("Layer Name: " + l_name)    
        l_outputs = _convert_layer(builder, l_name, l, inputs, l_outputs)
        inputs = l_outputs

        l_name = _gen_layer_name(l) + 'ADD'
        if i == (n-1):
            new_inputs = list(inputs)
            new_inputs.append(input_names[0])
            print(new_inputs)
            print(l_outputs)

            new_outputs = list(l_outputs)
            new_outputs[0] += 'ADD'
            l_outputs = _convert_cadd_table(builder, l_name, l, new_inputs, new_outputs)
            print(l_outputs)
            inputs = l_outputs
...
def _convert_convolution(builder, name, layer, input_names, output_names):
    ...
    k_h, k_w = layer.kernel_size[0], layer.kernel_size[1]

    pad_h, pad_w = layer.padding[0], layer.padding[1]
    weight = layer.weight.data.numpy().transpose((2, 3, 1, 0))
    ...
    builder.add_convolution(
        name=name,
        kernel_channels=layer.in_channels,
        output_channels=layer.out_channels,
        height=k_h,
        width=k_w,
        stride_height=layer.stride[0],
        stride_width=layer.stride[1],
        border_mode='valid',
        groups=1,
        W=weight,
        b=bias,
...
def _convert_full_convolution(builder, name, layer, input_names, output_names):
    ...
    weight = layer.weight.data.numpy().transpose((2, 3, 0, 1))
    bias = None
    if layer.bias is not None:
        bias = layer.bias.data.numpy()

    add_crop = False
    output_ = layer.output.datas.numpy()
    output_shape = (
...
def _convert_reflection_padding(builder,
                                ...
                                input_names,
                                output_names):
    pad_l = int(layer.padding[0])#.pad_l)
    pad_r = int(layer.padding[1])#pad_r)
    pad_t = int(layer.padding[2])#pad_t)
    pad_b = int(layer.padding[3])#pad_b)
...
def _convert_instance_norm(builder, name, layer, input_names, output_names):
    epsilon = layer.eps
    scale = layer.scale.data.numpy()
    shift = layer.shift.data.numpy()
...
_TORCH_LAYER_REGISTRY = {
    'UpsampleConvLayer': _convert_sequential,
    'ResidualBlock': _convert_residual,
    'ConvLayer': _convert_sequential,
    'TransformerNet': _convert_sequential,
    ...
    'InstanceNormalization': _convert_instance_norm,
    'ReflectionPad2d': _convert_reflection_padding,
    'SpatialReflectionPadding': _convert_reflection_padding,
    'SpatialUpSamplingNearest': _convert_upsampling_nearest,
    'Upsample': _convert_upsampling_nearest,
