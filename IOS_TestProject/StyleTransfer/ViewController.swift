...
class ViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    @IBAction func styleButtonTouched(_ sender: UIButton) {
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            ...
            DispatchQueue.main.async {
                ...
                //self.imageView.frame = CGRect(x: 0.0 ,y: 0.0, width: 512.0, height: 512.0);
    ...
    private func stylizeImage(cgImage: CGImage, model: MLModel) -> CGImage {
        ...
        let bytePerPixel = CVPixelBufferGetBytesPerRow(output)
        ...
        var a = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        var b = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        var c = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        var d = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        CVPixelBufferGetExtendedPixels(output, a, b, c, d)
    ...
    private func pixelBuffer(cgImage: CGImage, width: Int, height: Int) -> CVPixelBuffer {
        ...
        var c = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        var d = UnsafeMutablePointer<Int>.allocate(capacity: 8)
        CVPixelBufferGetExtendedPixels(pixelBuffer!, a, b, c, d)
